...
fn main() {
    ...
    let p1 = lines.iter().fold(0, |sum, s| {
        sum + s[1]
            .split(" ")
            .map(|s| s.len())
            .filter(|l| *l == 2 || *l == 3 || *l == 7 || *l == 4)
            .fold(0, |a, _| a + 1)
    });

    let overlaps = |haystack: &str, needle: &str| needle.split("").all(|c| haystack.contains(c));

    let mut p2 = 0;
    for line in &lines {
        let input = line[0];
        let output = line[1];
        let mut mappings: HashMap<i32, &str> = HashMap::new();

        // Need at most 3 runs to determine all numbers (longest chain is 4 -> 9 -> 5)
        for _ in 0..3 {
            for signal in input.split(" ") {
                match signal.len() {
                    2 => mappings.insert(1, signal),
                    3 => mappings.insert(7, signal),
                    4 => mappings.insert(4, signal),
                    7 => mappings.insert(8, signal),
                    5 => {
                        if mappings.contains_key(&1) && overlaps(signal, mappings.get(&1).unwrap())
                        {
                            mappings.insert(3, signal)
                        } else if mappings.contains_key(&9)
                            && overlaps(mappings.get(&9).unwrap(), signal)
                        {
                            mappings.insert(5, signal)
                        } else {
                            mappings.insert(2, signal)
                        }
                    }
                    6 => {
                        if !(mappings.contains_key(&1)
                            && overlaps(signal, mappings.get(&1).unwrap()))
                        {
                            mappings.insert(6, signal)
                        } else if mappings.contains_key(&4)
                            && overlaps(signal, mappings.get(&4).unwrap())
                        {
                            mappings.insert(9, signal)
                        } else {
                            mappings.insert(0, signal)
                        }
                    }
                    _ => None,
                };
        ...
        let mut out_num = 0;
        for out_signal in output.split(" ") {
            for (num, signal) in &mappings {
                if overlaps(out_signal, signal) && overlaps(signal, out_signal) {
                    out_num = out_num * 10;
                    out_num += num;
                    break;
                }
